...
function img_check(a, b, c, d) {
    ...
    0 != this.AllowExt && -1 == this.AllowExt.indexOf(this.FileExt) ? $("#" + c + "_error").text("只支持jpg、gif、png、jpeg格式，请重新上传").show() : ($("#" + c + "_error").text("").hide(), $.ajaxFileUpload({
        url: b,
        secureuri: !1,
        fileElementId: c,
        data: {
...
$(function() {
    ...
    null == $.cookie("viewTimes") ? ($("#stepTip img").attr("src", ctx + "/images/tiponce.jpg").parent("#stepTip").fadeIn(200), $.cookie("viewTimes", "1", b)) : (c = parseInt($.cookie("viewTimes")) + 1, $("#stepTip img").attr("src", ctx + "/images/tipagain.jpg").parent("#stepTip").fadeIn(200), $.cookie("viewTimes", c, b)),
    $(document).click(function() {
        var a = [];
        $("#box_industry li.current").length > 1 ? alert("行业领域最多可选1个") : ($("#box_industry li.current").each(function() {
            a.push($(this).text())
        }), "" == a.join() ? $("#select_industry").val("请选择行业领域") : $("#select_industry").val(a.join()), $("#select_industry_hidden").val(a.join()), $("#box_industry").hide()),
        $("#box_industry").hide(),
        $("#box_scale").hide(),
    ...
    function(a) {
        ...
        $(this).hasClass("current") ? ($(this).removeClass("current"), $("#box_industry li.current").each(function() {
            b.push($(this).text())
        }), $("#select_industry").val(b.join()), $("#select_industry_hidden").val(b.join())) : 0 == $(this).siblings("li.current").length ? ($(this).addClass("current"), b.push($(this).text()), $("#select_industry").val(b.join()), $("#select_industry_hidden").val(b.join())) : 0 == $(this).siblings("li.current").length ? ($(this).addClass("current"), $("#box_industry li.current").each(function() {
            b.push($(this).text())
        }), $("#select_industry").val(b.join()), $("#select_industry_hidden").val(b.join()), $("#box_industry").hide()) : $(this).siblings("li.current").length >= 1 && alert("行业领域最多可选1个"),
        $("#stepForm").validate().element("#select_industry_hidden")
    }),
    $("#select_scale").bind("click",
    function(a) {
        a.stopPropagation(),
        $("#box_scale").show(),
        $("#box_industry").hide(),
    ...
    $("#box_industry,#box_scale").bind("click",
    function(a) {
        a.stopPropagation()
    }),
    ...
    $("#stagesList").on("click", ".select_invest",
    function(a) {
        a.stopPropagation(),
        $("#box_scale").hide(),
        $("#box_industry").hide(),
    ...
    $("#stagesList").on("click", ".box_invest li",
    function(a) {
        var b, c;
        a.stopPropagation(),
        b = $(this).html(),
    ...
    $("#stagesList").on("change", ".select_invest",
    function() {
    ...
    $("#stepForm").validate({
        ...
        submitHandler: function(b) {
            ...
            $.ajax({
            	...
            	//alert(a)
                a.success ? window.location.href ="tag": window.location.href ="tag",//$("#beError").text(a.msg).show(),
                $(b).find(":submit").attr("disabled", !1)
            })
        }
    }),
